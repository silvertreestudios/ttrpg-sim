...
let _config;
let _onChange;
export function initSidebar(config, onChange) {
    _config = config;
    _onChange = onChange;
    renderFromConfig();
    bindStaticListeners();
}
export function updateSidebarConfig(config) {
    _config = config;
    renderFromConfig();
}
function emit() {
...
function toggleSubFields() {
    ...
    document.querySelectorAll('.cfg-lucky-sub').forEach(el => {
        el.style.display = luckyEnabled ? '' : 'none';
    });
    document.querySelectorAll('.cfg-piercer-sub').forEach(el => {
        el.style.display = piercerEnabled ? '' : 'none';
    });
}
function renderAttacksList() {
    ...
    _config.attacks.forEach((atk, idx) => {
      ...
      <div class="field-row">
        ...
        <select class="input-select atk-dice" data-idx="${idx}">
          ${['1d4', '1d6', '1d8', '1d10', '1d12', '2d6'].map(d => `<option value="${d}" ${d === atk.weapon.damageDice ? 'selected' : ''}>${d}</option>`).join('')}
        </select>
      </div>
      <div class="field-row">
        <label>Magic Bonus</label>
        <select class="input-select atk-magic" data-idx="${idx}">
          ${[0, 1, 2, 3].map(b => `<option value="${b}" ${b === atk.weapon.magicBonus ? 'selected' : ''}>+${b}</option>`).join('')}
        </select>
      </div>
      <div class="field-row">
    ...
    container.querySelectorAll('.atk-name').forEach(el => {
        el.addEventListener('input', (e) => {
            ...
            _config.attacks[idx].name = e.target.value;
            emit();
        });
    });
    container.querySelectorAll('.atk-dice').forEach(el => {
        el.addEventListener('change', (e) => {
            const idx = parseInt(e.target.dataset.idx);
            _config.attacks[idx].weapon.damageDice = e.target.value;
            emit();
        });
    });
    container.querySelectorAll('.atk-magic').forEach(el => {
        el.addEventListener('change', (e) => {
            const idx = parseInt(e.target.dataset.idx);
            _config.attacks[idx].weapon.magicBonus = parseInt(e.target.value);
            emit();
        });
    });
    container.querySelectorAll('.atk-abilmod').forEach(el => {
        el.addEventListener('change', (e) => {
            const idx = parseInt(e.target.dataset.idx);
            _config.attacks[idx].useAbilityMod = e.target.checked;
            emit();
        });
    });
    container.querySelectorAll('.atk-pact').forEach(el => {
        el.addEventListener('change', (e) => {
            const idx = parseInt(e.target.dataset.idx);
            _config.attacks[idx].isPactWeapon = e.target.checked;
            emit();
        });
    });
...
function renderRidersList() {
    ...
    // Bind rider inputs
    const bindRiderInput = (selector, field, transform) => {
        container.querySelectorAll(selector).forEach(el => {
            const evtType = el.tagName === 'INPUT' && el.type === 'checkbox'
                ? 'change' : 'input';
            el.addEventListener(evtType, (e) => {
                const idx = parseInt(e.target.dataset.idx);
                const raw = el.tagName === 'INPUT' && el.type === 'checkbox'
                    ? e.target.checked
                    : e.target.value;
                const val = transform ? transform(raw) : raw;
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                _config.riders[idx][field] = val;
                if (field === 'name')
                    renderRidersList();
                emit();
            });
...
function bindStaticListeners() {
    ...
    document.getElementById('btn-add-rider')?.addEventListener('click', () => {
        const newRider = {
            ...
            damage: '1d6',
            doublesOnCrit: false,
            condition: 'onEveryHit',
            placement: 'firstAvailable',
