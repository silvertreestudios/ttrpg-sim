...
// ============================================================
export function renderTable(containerId, columns, rows, caption) {
    const container = document.getElementById(containerId);
    if (!container)
        return;
    const thead = `<tr>${columns.map(c => `<th class="${c.alignRight ? 'text-right' : ''}">${c.header}</th>`).join('')}</tr>`;
...
export function highlightMax(getValue) {
    return (row, rows) => {
...
export function fmtDec(places) {
    return (val) => Number(val).toFixed(places);
}
/** Format as percentage */
export function fmtPct(places = 1) {
    return (val) => `${(Number(val) * 100).toFixed(places)}%`;
}
/** Format with sign */
export function fmtSigned(places = 1) {
    return (val) => {
        const n = Number(val);
...
export function renderDPRTable(containerId, acs, scenarios, breakpoints) {
    const container = document.getElementById(containerId);
    if (!container)
        return;
    const thead = `
  ...
  `;
    const tbody = acs.map((ac, idx) => {
        ...
        // Breakpoint styling
        let rowClass = '';
        if (ac === breakpoints.allSSvsSkip)
            rowClass = 'row-breakpoint';
        else if (ac === breakpoints.skipSSvsNoSS)
            rowClass = 'row-breakpoint';
