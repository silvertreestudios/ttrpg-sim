...
/**
 * Build triple advantage distribution (Elven Accuracy) from a base straight dist.
 * Elven Accuracy upgrades advantage to three dice, take max.
 */
export function tripleAdvantageDist(base: D20Dist): D20Dist {
  const prob = new Array(21).fill(0);
  // P(max = k) = CDF(k)^3 - CDF(k-1)^3
  for (let k = 1; k <= 20; k++) {
    prob[k] = base.cdf[k] ** 3 - base.cdf[k - 1] ** 3;
  }
...
export function getD20Dist(
  ...
  if (advState === 'advantage') return elvenAccuracy ? tripleAdvantageDist(base) : advantageDist(base);
